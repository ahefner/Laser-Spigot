...
(defmacro do-2d (((row-var rows) (col-var cols) &rest row-vars) &body body)
  `(flet ((body (,row-var ,col-var ,@(mapcar 'first row-vars)) ,@body))
     (declare (inline body))
     (let ((rows ,rows) (cols ,cols))
       (dotimes (row rows)
         (let ,row-vars
           (dotimes (col cols)
             (body row col ,@(mapcar 'first row-vars))))))))

(defmacro do-matrix ((array (elt-var &optional row-var col-var) &rest row-vars) &body body)
  (setf elt-var (or elt-var (gensym "ELT")))
  (setf row-var (or row-var (gensym "ROW")))
  (setf col-var (or col-var (gensym "COL")))
  `(do-2d ((,row-var (array-dimension ,array 0))
           (,col-var (array-dimension ,array 1))
           ,@row-vars)
     (symbol-macrolet ((,elt-var (aref ,array ,row-var ,col-var)))
       ,@body)))
