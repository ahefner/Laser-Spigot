...
struct cached_face
{
    ...
    /* This is ridiculously wasteful. It was resonable when I only did ASCII. */
    struct cached_glyph glyphs[0x2FFF];
};
...
ensure_cached_face (unsigned facenum, unsigned text_height)
{
    ...
    printf("For your amusement: sizeof(cached_faces) = %i\n", sizeof(cached_faces));

    return f;
}

static struct cached_glyph *
ensure_cached_glyph (struct cached_face *cface, unsigned code, FT_UInt index)
{
...
image_t render_label_8 (unsigned facenum, uint32_t color, unsigned text_height, unsigned char *string)
{
    ...
    unsigned *tmp = malloc(sizeof(unsigned) * len);
    assert(tmp != NULL);
    for (int i=0; i<len; i++) tmp[i] = string[i];
    image_t img = render_label_32(facenum, color, text_height, tmp);
    free(tmp);
    return img;
}
...
image_t render_glyph(unsigned facenum, uint32_t color, unsigned text_height, unsigned glyph)
{
    ...
    g[0] = glyph;
    g[1] = 0;
    printf("Render glyph %i\n", glyph);
    return render_label_32(facenum, color, text_height, &g);
}
...
image_t render_label_32 (unsigned facenum, uint32_t color, unsigned text_height, unsigned *string)
{
    ...
    if (data == NULL) {
        printf("Can't allocate final memory for size %i label\n", text_height);
        printf("Final bounds: %i %i %i %i baseline=%i\n", 0, min_y, max_x, max_y, baseline);
        return NULL;
    } else {
