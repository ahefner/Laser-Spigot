...
;;  (c "glClear(GL_COLOR_BUFFER_BIT)")
  (check-gl-error "paint-begin pre")
  (c "glDisable(GL_DEPTH_TEST)")
  (check-gl-error "Disable depth test")
  (c "glDisable(GL_CULL_FACE)")
  (check-gl-error "Disable culling")
  (c "glShadeModel(GL_FLAT)")
  (check-gl-error "Set shading model")
  (c "glEnable(GL_BLEND)")
  (set-blend-mode :blend)
  (check-gl-error "Configure blending")
  (c "glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE)")
  (check-gl-error "Texture environment")
  (c "glMatrixMode(GL_PROJECTION)")
  (c "glLoadIdentity()")
  (check-gl-error "Reset projection")
  (c "glOrtho(0, window_width, window_height, 0, -1, 1)")
  (check-gl-error "Set transformations"))
...
(defun render-label (owner face height string &key (align-x :left) (align-y :baseline))
  (let* ((facenum (ecase face
                    ...
                    (:bold-italic 3)
                    (:gothic 4)))
         (cimage (call :pointer-void "render_label_8"
...
(defun render-glyph (owner face height code &key (align-x :center) (align-y :baseline))
  (when (characterp code) (setf code (char-code code)))
  (let* ((facenum (ecase face
                    (:sans 0)
                    (:bold 1)
                    (:italic 2)
                    (:bold-italic 3)
                    (:gothic 4)))
         (cimage (call :pointer-void "render_glyph"
                       :unsigned-int facenum
                       :unsigned-int #xFFFFFF :unsigned-int height :unsigned-int code))
         (img (make-img :width  (cx :int "((image_t)#0)->w" :pointer-void cimage)
                        :height (cx :int "((image_t)#0)->h" :pointer-void cimage)
                        :name (list :glyph code)
                        :owner owner
                        :pixels (cx :pointer-void "((image_t)#0)->pixels" :pointer-void cimage)
                        :x-offset (cx :int "((image_t)#0)->x_origin" :pointer-void cimage)
                        :y-offset (cx :int "((image_t)#0)->y_origin" :pointer-void cimage))))    
...
(defun center-img (img)
  (setf (img-x-offset img) (ash (img-width img) -1))
  img)
